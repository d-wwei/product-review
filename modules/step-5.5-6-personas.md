...
>
> **数据外置**：完整档案模板和行为指令集模板也独立存放在 `modules/persona-templates.md`。
> 该文件可在 context 紧张时作为轻量替代加载路径——subagent 只 Read 外置文件而非本文件全文。
> 本文件保留所有内容作为权威参考，两者内容一致。
...
> 当模式为 CONTENT_REVIEW 或设置了 --content-review 标志时，对每个 Persona 追加内容消费特征。
> 此增强不改变现有 Persona 生成流程，只在其之上追加属性。

**追加属性**：
- **阅读习惯**：深度长文 / 快速扫描 / 标题党敏感 / 偏好视频 / 偏好图文
- **兴趣偏好**：{根据产品领域动态生成，如"时政/科技/娱乐/财经/体育"}
- **内容格式偏好**：短视频 / 长视频 / 图文 / 纯文字 / 直播 / 播客
- **互动倾向**：纯消费者 / 偶尔评论 / 活跃创作者 / 社交分享者
- **内容消费原型**：{5 种预设之一}
  - 深度阅读者：追求完整信息，看完全文再评价
  - 碎片浏览者：通勤/睡前刷，3 秒决定是否点入
  - 垂直专家：只关注特定领域，对低质量容忍度极低
  - UGC 社交用户：重互动和社区氛围，内容只是社交载体
  - 冷启动新用户：无历史偏好，考验推荐系统和内容发现

**追加正交约束**：
1. 至少 3 种不同阅读习惯
2. 至少 1 个冷启动新用户（测试推荐系统对新用户的表现）
3. 深度阅读者和碎片浏览者必须同时存在（测试内容深度光谱）

**Persona 模拟后额外产出**（注入到每个 Persona 的反馈模板中）：
- Feed 前 20 条逐条打分（1-5），标注问题内容（标题党/低质/重复/过期/软广）
- 内容满意度（1-10）+ 一句话评价
- 发现效率（1-5）：找到感兴趣内容的难度
...
**外置数据文件**：完整档案模板和行为指令集模板也存放在 `modules/persona-templates.md`（148 行）。
当 subagent 只需要模板结构（而非本文件的完整执行指导）时，可直接 Read 该文件以节省 context。
下方的内联模板是权威版本，外置文件与其保持同步。
