...
- **主 agent context 保护**：subagent 的完整报告写入 `exploration-reports/` 目录下的独立文件，返回给主 agent 的只是不超过 20 行的摘要。报告生成由主 agent 直接从文件读取探索数据并分段写入
...
> **用途**：创建多台模拟器同时运行，配合 Step 5 的多设备并行分派，
> 将探索时间缩短 2-3 倍。
...
# 创建 2-3 台模拟器（建议不同型号以测试多分辨率）
xcrun simctl create "PR-iPhone15Pro" "com.apple.CoreSimulator.SimDeviceType.iPhone-15-Pro" "com.apple.CoreSimulator.SimRuntime.iOS-18-2"
xcrun simctl create "PR-iPhone15" "com.apple.CoreSimulator.SimDeviceType.iPhone-15" "com.apple.CoreSimulator.SimRuntime.iOS-18-2"
xcrun simctl create "PR-iPhoneSE" "com.apple.CoreSimulator.SimDeviceType.iPhone-SE-3rd-generation" "com.apple.CoreSimulator.SimRuntime.iOS-18-2"

# 同时启动
xcrun simctl boot PR-iPhone15Pro
xcrun simctl boot PR-iPhone15
open -a Simulator
```
...
# 在每台上安装目标 App（.app 文件）
xcrun simctl install PR-iPhone15Pro <path-to-app>
xcrun simctl install PR-iPhone15 <path-to-app>
```

**性能建议**：
- 建议最多同时运行 2-3 台模拟器（受 Mac 内存限制）
- 16GB RAM → 最多 2 台
- 32GB+ RAM → 可以 3 台
- 每台模拟器约占 2-4GB 内存
