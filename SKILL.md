...
Agent(
    ...
    4. **子页面递归**：进入子页面后重复步骤 2-3
    5. 用 mobile_list_elements_on_screen 获取元素信息，优先于纯截图分析
    6. 记录以下内容：
       - 功能描述
       - 操作路径（每一步点击了什么）
    ...
    这是最容易偷懒的环节。以下是强制执行的操作序列：

    ```
    对当前页面执行：

    STEP A: 初始快照
      → mobile_list_elements_on_screen → 记录为 snapshot_0
      → mobile_save_screenshot（命名：{page}-scroll-0-top.png）
      → 记录 snapshot_0 的元素数量 = N0

    STEP B: 滚动循环（至少执行 3 次，不论是否看到新内容）
      → scroll_count = 0
      → same_count = 0   // 连续未变化计数器
      → LOOP:
          → mobile_swipe_on_screen direction: up
          → mobile_list_elements_on_screen → 记录为 snapshot_{scroll_count+1}
          → scroll_count += 1
          → 比较 snapshot_{scroll_count} 与 snapshot_{scroll_count-1}：
              如果有新元素出现 → same_count = 0
              如果完全相同 → same_count += 1
          → 如果 scroll_count < 3 → 继续 LOOP（强制最少 3 次滚动）
          → 如果 scroll_count >= 3 且 same_count >= 2 → 退出 LOOP
          → 如果 scroll_count >= 15 → 退出 LOOP（安全上限，防止无限滚动列表）
          → 否则 → 继续 LOOP

    STEP C: 底部快照
      → mobile_save_screenshot（命名：{page}-scroll-{scroll_count}-bottom.png）
      → 记录底部元素列表为 snapshot_final

    STEP D: 输出滚动摘要（必须写入探索报告）
      → "页面: {page_name}"
      → "总滚动次数: {scroll_count}"
      → "首屏元素数: {N0}"
      → "末屏元素数: {N_final}"
      → "发现的新元素总数: {滚动后出现的、首屏没有的元素数量}"
      → "到达底部: {是/否}"
      → "底部标志: {版权信息/加载完毕/空白/无限列表截断}"
    ```

    **硬性规则**：
    - 每个页面最少滚动 3 次，即使第一次滚动后看起来"到底了"
    - "连续 2 次元素完全相同"才算到底，不是"看起来差不多"
    - 滚动摘要必须出现在探索报告中——没有摘要 = 没有执行滚动
    - 如果首屏元素数 > 10 且只滚动了 3 次就到底，在摘要中标注 [需复查]
    ...
    ## 探索完成标准（必须全部满足，且每项有可验证证据）
    ✅ 每个页面有滚动摘要（含滚动次数、首屏/末屏元素数）
    ✅ 每个页面至少截图 2 张（scroll-0-top + scroll-N-bottom）
    ✅ 每个页面上的所有入口/按钮/链接都已点击并记录
    ✅ 每个子页面也已执行滚动扫描协议
    ✅ 每个可交互功能已实际操作使用（而不只是查看）
    ✅ 报告包含第一人称的实际使用感受
    ✅ 报告末尾有完整的【滚动覆盖汇总表】（见下方格式）
    ...
    | 页面 | 滚动次数 | 首屏元素数 | 末屏元素数 | 新增元素数 | 到底 | 底部标志 | 备注 |
    |------|---------|-----------|-----------|-----------|------|---------|------|
    | {页面名} | {N} | {N0} | {Nf} | {新增} | 是/否 | {标志} | {[需复查]等} |
    | {子页面} | {N} | {N0} | {Nf} | {新增} | 是/否 | {标志} | |
    | ... | | | | | | | |
    | **汇总** | **{总次数}** | | | **{总新增}** | **{X}/{Y}到底** | | |

    没有这张表 = 探索不合格。主 agent 收到报告后会检查此表。
    如果任何页面滚动次数为 0 或 1，主 agent 会要求该 subagent 重新探索。
    """,
...
STEP B: 滚动循环
  ...
  → LOOP:
      ...
      → 安全上限：scroll_count >= 15 强制退出（防无限列表）
      → 否则继续 LOOP
...
5. **无限列表处理**：如果 15 次滚动后仍有新内容，标注 `[无限列表，已截断于第15屏]`，记录已发现的元素特征即可
...
| 页面 | 滚动次数 | 首屏元素 | 末屏元素 | 新增元素 | 到底 | 备注 |
|------|---------|---------|---------|---------|------|------|
| {页面名} | {N} | {N0} | {Nf} | {new} | 是/否 | |

没有此表 = 探索不合格。主 agent 会拒收并要求重做。
```
